...
async function updateStatus() {
    try {
        ...
        if (currentOptionsCount <= 1) {
            select.innerHTML =
                '<option value="" disabled selected>-- Select boot configuration --</option>'
            configs.forEach((cfg) => {
...
async function updateLogs() {
    try {
        ...
        if (logs.length === 0) {
            logsList.innerHTML =
                '<div class="log-entry system">No events logged yet.</div>'
            return
        }
...
document
    .getElementById("cancel-override-btn")
...
document.getElementById("clear-logs-btn").addEventListener("click", () => {
    document.getElementById("logs-list").innerHTML =
        '<div class="log-entry system">View cleared. Logging resumes...</div>'
})
