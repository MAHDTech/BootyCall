...
setup_overlayfs() {
	...
	if mount -t squashfs "$IMG_FILE" /mnt/squashfs; then
		...
		if [ ! -d /mnt/squashfs/bin ] || [ ! -s /mnt/squashfs/bin/busybox ]; then
			echo "ERROR: The Squashfs mount isempty. Is it a corrupt img? Check MD5."
			echo "Showing current squashfs directory contents:"
			ls -l /mnt/squashfs
			umount /mnt/squashfs
			drop_to_shell
			return 1
		...
		echo "Squashfs populated: $(find /mnt/squashfs -type f | wc -l) files."
		mkdir -p /overlay
...
setup_nw() {
	if [ "$IPV6" = "true" ] || [ -n "$PHOENIX_IP" ]; then
		...
		if ! setup_uplink_to_foundation; then drop_to_shell; fi
	else
...
. "${HOME}/ce_functions.sh" || {
	echo "Failed to source CE functions!"
	drop_to_shell
}
